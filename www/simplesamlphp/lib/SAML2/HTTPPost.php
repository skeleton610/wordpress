...
class SAML2_HTTPPost extends SAML2_Binding {
	...
	public function send(SAML2_Message $message) {
        ...
        SimpleSAML_Logger::debug('****************** in  HTTP_POST send *******************');
        ...
        SimpleSAML_Logger::debug('****** in  HTTP_POST send. toSignedXML ***********'.$msgStr);

        $msgStr = $msgStr->ownerDocument->saveXML($msgStr);

        SimpleSAML_Logger::debug('***** in  HTTP_POST send. before encode *********'.$msgStr);

		SimpleSAML_Utilities::debugMessage($msgStr, 'out');

        $msgStr = base64_encode($msgStr);

        SimpleSAML_Logger::debug('****** in  HTTP_POST send. after encode ********'.$msgStr);
